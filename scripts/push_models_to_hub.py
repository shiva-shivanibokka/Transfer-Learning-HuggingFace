...
def push_vision(user: str, log):
    ...
    for model_key, repo_name in VISION_REPO.items():
        ...
        model.push_to_hub(repo_id, commit_message="EuroSAT fine-tuned vision model")
        processor.push_to_hub(repo_id, commit_message="image processor")
        upload_file(
            path_or_fileobj=str(ckpt.parent / "result.json"),
...
def push_text(user: str, log):
    ...
    for model_key, repo_name in TEXT_REPO.items():
        ...
        model.load_state_dict(torch.load(ckpt, map_location="cpu", weights_only=True))
        log.info("pushing %s -> %s", model_key, repo_id)
        model.push_to_hub(repo_id, commit_message="dair-ai/emotion fine-tuned")
        tok.push_to_hub(repo_id, commit_message="tokenizer")
...
def main():
    ...
    if not args.skip_vision:
        push_vision(args.user, log)
    if not args.skip_text:
        push_text(args.user, log)
